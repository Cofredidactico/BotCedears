...
El núcleo de cálculo (`indicators.js` y `scoring.js` — funciones puras, sin
dependencias) tiene una red de tests con el runner nativo de Node (sin
...
Cubren indicadores (EMA/SMA/RSI/ATR/MACD/Bollinger/OBV), `computeTechnical`,
`computeScore` (incluye un test de comportamiento: una tendencia alcista debe
puntuar más que una bajista), `computePlan` y el contrato de
`detectPriceAlert`. El `package.json` es **solo para estas herramientas de
Node**: no agrega dependencias ni pasos de build, y el frontend se sigue
sirviendo como estáticos igual que antes.
...
## Análisis didáctico + noticias en español

- **Los cuadros densos de "Análisis Técnico" y "Análisis Fundamental" de la
  ficha se reemplazaron por tarjetas didácticas** ("¿Cómo viene el precio?" y
  "¿Cómo está la empresa?"): cada punto es un veredicto con semáforo
  (👍 bien / ⚠️ atención / 👎 mal) y una frase en lenguaje llano — tendencia,
  momentum, fuerza vs mercado, volumen y nivel de precio en lo técnico;
  crecimiento, rentabilidad, "¿cara o barata?" (vs. su sector), deuda,
  dividendos y próximo balance en lo fundamental. Derivan de los MISMOS datos
  reales que antes, solo que interpretados. La tabla completa de indicadores
  queda disponible en un desplegable "Ver todos los indicadores (avanzado)"
  para quien quiera el detalle. Si no hay cobertura fundamental, se muestra un
  mensaje claro en vez de una tabla vacía.
- **Noticias traducidas al español**: los titulares (que llegan en inglés de
  Finnhub) se traducen vía el endpoint gratuito de Google Translate en una
  sola llamada por lista. El sentimiento (Positiva/Negativa/…) se sigue
  calculando sobre el titular original en inglés para no perder precisión, y
